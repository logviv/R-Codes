...
#	Import the Bollywood data set in Rstudio in a variable named bollywood
  ...
  bollywood <-read.csv(file.choose("bollywood.csv"),header=T)
...
# Store the movie name in variable named top_movie
  ...
  top_movie <-as.character(bollywood[which.max(bollywood$Tcollection),1])
...
# You can use	a column named 'Tcollection' for this 
  ...
  shahrukh_collection <- sum(sharukh$Tcollection)
    
	akshay_collection <- sum(Akshay$Tcollection)
    
	amitabh_collection <- sum(Amithab$Tcollection)
...
# Store the names of 4 movies in same sequence in movie_result vector
  ...
  movie_result <-list(bollywood$ocollection,bollywood$wcollection,bollywood$Fcollection,bollywood$Tcollection)
  movie_result <-bollywood[sapply(movie_result,which.max),1]
